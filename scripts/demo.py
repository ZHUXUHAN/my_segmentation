...
parser.add_argument('--inputdir', type=str,
                    default='./demo_inputdir',
                    help='path to the input picture')
parser.add_argument('--outdir', default='./demo_outdir', type=str,
...
def demo(config):
    ...
    model = get_model(args.model, pretrained=True, root=args.save_folder, local_rank=args.local_rank, iteration=args.iteration).to(device)
    ...
    for i, file in enumerate(files):
        ...
        with torch.no_grad():
            ...
            path = os.path.join(args.inputdir, file)
            ori_image = cv2.imread(path)
            ori_image = ori_image.astype(np.float32)
