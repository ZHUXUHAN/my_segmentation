...
__all__ = ['DeepLabV3', 'get_deeplabv3', 'get_deeplabv3_resnet50_voc', 'get_deeplabv3_resnet101_voc',
           ...
           'get_deeplabv3_resnet152_ade', 'get_deeplabv3_resnet50_mask', 'get_deeplabv3_resnet18_mask']
...
class DeepLabV3(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet50', aux=False, pretrained_base=True, **kwargs):
        ...
        if backbone!='resnet18':
            ...
            self.in_dim = 512
        self.head = _DeepLabHead(nclass, self.in_dim, **kwargs)
        if self.aux:
...
class _DeepLabHead(nn.Module):
    def __init__(self, nclass, in_dim = 2048, norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        super(_DeepLabHead, self).__init__()
        self.aspp = _ASPP(in_dim, [12, 24, 36], norm_layer=norm_layer, norm_kwargs=norm_kwargs, **kwargs)
...
def get_deeplabv3(dataset='pascal_voc', backbone='resnet50', pretrained=False, root='~/.torch/models',
    ...
    if pretrained:
        ...
        print('deeplabv3_%s_%s_%s' % (backbone, acronyms[dataset],iteration))
        model.load_state_dict(torch.load(get_model_file('deeplabv3_%s_%s_%s' % (backbone, acronyms[dataset], iteration), root=root),
                              map_location=device))
    return model
...
def get_deeplabv3_resnet50_mask(**kwargs):
    return get_deeplabv3('mask', 'resnet50', **kwargs)


def get_deeplabv3_resnet18_mask(**kwargs):
    return get_deeplabv3('mask', 'resnet18', **kwargs)
