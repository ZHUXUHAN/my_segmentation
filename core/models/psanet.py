...
__all__ = ['PSANet', 'get_psanet', 'get_psanet_resnet50_voc', 'get_psanet_resnet101_voc',
           ...
           'get_psanet_resnet152_citys' ,'get_psanet_resnet18_mask']
...
class PSANet(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet', aux=False, pretrained_base=True, **kwargs):
        ...
        if backbone != 'resnet18':
            ...
            self.in_dim = 512
        self.head = _PSAHead(nclass, self.in_dim, **kwargs)
        if aux:
...
class _PSAHead(nn.Module):
    def __init__(self, nclass, in_dim,norm_layer=nn.BatchNorm2d, **kwargs):
        super(_PSAHead, self).__init__()
        # psa_out_channels = crop_size // 8 ** 2
        self.psa = _PointwiseSpatialAttention(in_dim, 3600, norm_layer)
...
def get_psanet_resnet18_mask(**kwargs):
    return get_psanet('mask', 'resnet18', **kwargs)
